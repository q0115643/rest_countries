...
export function reducer(state: State = initialState, action: Action) {
  switch (action.type) {
    ...
    case ActionTypes.DEL_COUNTRY: {
      ...
      let alpha2Code = action.payload.alpha2Code.toUpperCase();
      let countries = state.countries.filter( country => { 
          return country.alpha2Code.toUpperCase() !== alpha2Code;
      })
      let foundCountries = state.foundCountries.filter( country => {
        return country.alpha2Code.toUpperCase() !== alpha2Code;
      })
    ...
    case ActionTypes.SEARCH_COUNTRIES: {
      ...
      let keyword = state.keyword.toUpperCase();
      let countries: Array<Country> = []
      for (let i = 0; i < state.countries.length; i++) {
        let country = state.countries[i];
    ...
    case ActionTypes.SORT_COUNTRIES: {
      ...
      let rule = action.payload.rule || "name";
      let countriesCopy = state.foundCountries.slice();
      if (rule === "name") {
        countriesCopy.sort((a, b) => {return compare(a, b, a.name, b.name, rule, nextDirection);})
      } else if (rule == "capital") {
        countriesCopy.sort((a, b) => {return compare(a, b, a.capital, b.capital, rule, nextDirection);})
      } else if (rule == "region") {
        countriesCopy.sort((a, b) => {return compare(a, b, a.region, b.region, rule, nextDirection);})
      } else if (rule == "call") {
        countriesCopy.sort((a, b) => {return compare(a, b, parseInt(a.callingCodes[0]), parseInt(b.callingCodes[0]), rule, nextDirection);})
      } else if (rule == "alpha2") {
        countriesCopy.sort((a, b) => {return compare(a, b, a.alpha2Code, b.alpha2Code, rule, nextDirection);})
      }
    ...
    case ActionTypes.ADD_COUNTRY: {
      let addOpen: boolean = true;
      let colorRed: boolean = false;
      let countries: Array<Country> = state.countries.slice();
      let foundCountries: Array<Country> = state.foundCountries.slice();
